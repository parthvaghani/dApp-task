...
import {
    executeBatchedApprovalAndTransfer,
    executeSequentialApprovalAndTransfer
...
import { Zap, Wallet, Copy, Layers, ArrowRight } from "lucide-react";

type TransactionStatus = "idle" | "pending" | "success" | "error";
type BatchMode = "batched" | "sequential";
...
export default function BatchedActions({ isLoggedIn }: BatchedActionsProps) {
    ...
    const [batchMode, setBatchMode] = useState<BatchMode>("batched");
    ...
    const handleBatchedAction = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let result;
            if (batchMode === "batched") {
                result = await executeBatchedApprovalAndTransfer(
                ...
                result = await executeSequentialApprovalAndTransfer(
                    spender as `0x${string}`,
                    recipient as `0x${string}`,
                    amountWei
                );
                setTxHash(result.transferHash);
            }

            setStatus("success");
            toast.success(
                batchMode === "batched"
                    ? "Smart approval + transfer successful!"
                    : "Sequential approval + transfer successful!"
            );
    ...
    const fillTestData = () => {
        setSpender("0x742d35Cc6634C0532925a3b8D8C8EC29582f6442");
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h4 className="font-semibold text-gray-900 mb-4">Transaction Mode</h4>
                <div className="grid grid-cols-2 gap-3">
                    <button
                        type="button"
                        onClick={() => setBatchMode("batched")}
                    ...
                    >
                        ...
                        <p className="text-xs text-gray-600">Auto-detect & execute in one transaction</p>
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        <p className="text-xs text-gray-600">Approve first, then transfer separately</p>
                    </button>
                </div>
            </div>
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            {isLoading ? (
                                ...
                                <>
                                    <Zap className="w-5 h-5" />
                                    Execute {batchMode === "batched" ? "Smart" : "Sequential"} Action
                                </>
                            )}
                        </button>
                        <button
            ...
            <div className="bg-gray-50 rounded-xl p-4">
                <h5 className="font-medium text-gray-900 mb-2">
                    {batchMode === "batched" ? "Smart Mode" : "Sequential Mode"} Benefits
                </h5>
                {batchMode === "batched" ? (
                    ...
                    <div className="space-y-2 text-sm">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-700">Two separate transactions</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                            <span className="text-gray-700">Explicit approval step</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                            <span className="text-gray-700">Better for complex scenarios</span>
                        </div>
                    </div>
                )}
            </div>
